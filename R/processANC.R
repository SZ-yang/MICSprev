#' Run full ANC 4+ analysis (direct, FH, unit, stratified + plots/tables)
#'
#' @param anc_data     Data frame created by process_ANC()
...
run_anc_analysis <- function(
  ...
  cat("\n=== ANC Direct Estimates ===\n")

  # ---- direct estimates ----
  anc_res_adm0 <- surveyPrev::directEST(
    data         = anc_data,
    cluster.info = cluster.info,
    admin        = 0
  )
  cat("National ANC 4+ coverage:",
      round(anc_res_adm0$res.admin0$direct.est, 4), "\n")

  anc_res_adm1 <- surveyPrev::directEST(
    data         = anc_data,
    cluster.info = cluster.info,
    admin        = 1
  )

  anc_res_adm2 <- surveyPrev::directEST(
    data         = anc_data,
    cluster.info = cluster.info,
    admin        = 2
  )
  ...
  # ---- Fay-Herriot admin1 ----
  anc_FH_adm1 <- surveyPrev::fhModel(
    data         = anc_data,
    cluster.info = cluster.info,
  ...
  # ---- Fay-Herriot admin2 ----
  bad_admin2 <- subset(anc_res_adm2$res.admin2,
  ...
  anc_FH_adm2 <- surveyPrev::fhModel(
    data         = subset(anc_data, !cluster %in% bad_clusters),
    cluster.info = cluster.info,
  ...
  anc_unit_adm1 <- surveyPrev::clusterModel(
    data           = anc_data,
    cluster.info   = cluster.info,
    admin.info     = admin.info1,
  ...
  anc_unit_adm2 <- surveyPrev::clusterModel(
    data           = anc_data,
    cluster.info   = cluster.info,
    admin.info     = admin.info2,
    stratification = FALSE,
  ...
  anc_unit_strat_adm1 <- surveyPrev::clusterModel(
    data           = anc_data,
    cluster.info   = cluster.info,
    admin.info     = admin.info1,
    stratification = TRUE,
  ...
  anc_unit_strat_adm2 <- surveyPrev::clusterModel(
    data           = anc_data,
    cluster.info   = cluster.info,
    admin.info     = admin.info2,
    stratification = TRUE,
  ...
  p1 <- surveyPrev::intervalPlot(
    ...
    compare = TRUE,
    model   = list(
      "Direct Estimates"          = anc_res_adm1,
      "Fay-Herriot Model"         = anc_FH_adm1,
      "Cluster-Level Model"       = anc_unit_adm1,
      "Stratified Cluster Model"  = anc_unit_strat_adm1
    )
  ) +
  ...
  p2 <- surveyPrev::intervalPlot(
    ...
    compare = TRUE,
    model   = list(
      "Direct Estimates"          = anc_res_adm2,
      "Fay-Herriot Model"         = anc_FH_adm2,
      "Cluster-Level Model"       = anc_unit_adm2,
      "Stratified Cluster Model"  = anc_unit_strat_adm2
    )
  ) +
    ggplot2::ylab("") + ggplot2::xlab("") +
    ggplot2::scale_color_discrete("") +
  ...
  ## ---------------- Map plots ----------------
  # Admin1 maps
  out1 <- anc_res_adm1$res.admin1[, c("admin1.name", "direct.est", "cv")]
  colnames(out1)[2] <- "mean"
  out1$model <- "Direct Estimates"

  out2 <- anc_FH_adm1$res.admin1[, c("admin1.name", "mean", "cv")]
  out2$model <- "Fay-Herriot Model"

  out3 <- anc_unit_adm1$res.admin1[, c("admin1.name", "mean", "cv")]
  out3$model <- "Cluster-Level Model"

  out4 <- subset(anc_unit_strat_adm1$res.admin1, type == "full")[
  ...
  # Admin2 maps
  out1 <- anc_res_adm2$res.admin2[, c("admin2.name.full", "direct.est", "cv")]
  colnames(out1)[2] <- "mean"
  out1$model <- "Direct Estimates"

  out2 <- anc_FH_adm2$res.admin2[, c("admin2.name.full", "mean", "cv")]
  out2$model <- "Fay-Herriot Model"

  out3 <- anc_unit_adm2$res.admin2[, c("admin2.name.full", "mean", "cv")]
  out3$model <- "Cluster-Level Model"

  out4 <- subset(anc_unit_strat_adm2$res.admin2, type == "full")[
  ...
  s1 <- surveyPrev::scatterPlot(
    res1    = anc_res_adm1$res.admin1,
    res2    = anc_FH_adm1$res.admin1,
    value1  = "direct.est",
    value2  = "mean",
    by.res1 = "admin1.name",
    by.res2 = "admin1.name",
  ...
  s5 <- surveyPrev::scatterPlot(
    res1    = anc_res_adm1$res.admin1,
    res2    = subset(anc_unit_strat_adm1$res.admin1, type == "full"),
    value1  = "direct.est",
    value2  = "mean",
    by.res1 = "admin1.name",
    by.res2 = "admin1.name",
  ...
  # Admin2
  s1 <- surveyPrev::scatterPlot(
    res1    = anc_res_adm2$res.admin2,
    res2    = anc_FH_adm2$res.admin2,
    value1  = "direct.est",
    value2  = "mean",
  ...
  s5 <- surveyPrev::scatterPlot(
    res1    = anc_res_adm2$res.admin2,
    res2    = subset(anc_unit_strat_adm2$res.admin2, type == "full"),
    value1  = "direct.est",
    value2  = "mean",
    by.res1 = "admin2.name.full",
    by.res2 = "admin2.name.full",
  ...
  p7 <- ggplot2::ggplot() +
    ...
    ggplot2::ggtitle(paste("Cluster Locations - ANC 4+ Visits -", country_label))
  ggplot2::ggsave(
  ...
  p7b <- ggplot2::ggplot(geo2) +
    ...
    ggplot2::theme_bw() +
    ggplot2::theme(legend.position = "none") +
    ggplot2::ggtitle("Cluster Locations by Urban/Rural Strata - ANC")
  ggplot2::ggsave(
  ...
  ## ---------------- Ridge plots ----------------
  p9a <- surveyPrev::ridgePlot(x = anc_FH_adm1, direction = -1) +
    ggplot2::ggtitle("Fay-Herriot Model - ANC 4+ Coverage")
  p9b <- surveyPrev::ridgePlot(x = anc_unit_adm1, direction = -1) +
    ggplot2::ggtitle("Cluster-Level Model - ANC 4+ Coverage")
  p9c <- surveyPrev::ridgePlot(x = anc_unit_strat_adm1, direction = -1) +
    ggplot2::ggtitle("Stratified Cluster-Level Model - ANC 4+ Coverage")
  p9 <- p9a + p9b + p9c
  ggplot2::ggsave(
  ...
  p10 <- surveyPrev::ridgePlot(x = anc_FH_adm2, direction = -1)
  p11 <- surveyPrev::ridgePlot(x = anc_unit_adm2, direction = -1)
  p12 <- surveyPrev::ridgePlot(x = anc_unit_strat_adm2, direction = -1)
  ...
  ## ---------------- Rank plots ----------------
  p13a <- surveyPrev::rankPlot(x = anc_FH_adm1, direction = 1) +
    ggplot2::ggtitle("Fay-Herriot Model")
  p13b <- surveyPrev::rankPlot(x = anc_unit_adm1, direction = 1) +
    ggplot2::ggtitle("Cluster-Level Model")
  p13c <- surveyPrev::rankPlot(x = anc_unit_strat_adm1, direction = 1) +
  ...
  p14 <- surveyPrev::rankPlot(x = anc_FH_adm2, direction = 1)
  p15 <- surveyPrev::rankPlot(x = anc_unit_adm2, direction = 1)
  p16 <- surveyPrev::rankPlot(x = anc_unit_strat_adm2, direction = 1)
  ...
  p17 <- surveyPrev::exceedPlot(
    anc_FH_adm1, threshold = low_cov_threshold,
    exceed = FALSE, direction = 1,
    geo = geo_admin1, by.geo = "NAME_2", ylim = c(0, 1)
  ) + ggplot2::ggtitle(
    paste0("Fay-Herriot Model: Prob. ANC 4+ < ", thr_label)
  )

  p18 <- surveyPrev::exceedPlot(
    anc_FH_adm2, threshold = low_cov_threshold,
    exceed = FALSE, direction = 1,
  ...
  p19 <- surveyPrev::exceedPlot(
    anc_unit_adm1, threshold = low_cov_threshold,
    exceed = FALSE, direction = 1,
    geo = geo_admin1, by.geo = "NAME_2", ylim = c(0, 1)
  ) + ggplot2::ggtitle(
    paste0("Cluster-Level Model: Prob. ANC 4+ < ", thr_label)
  )

  p20 <- surveyPrev::exceedPlot(
    anc_unit_adm2, threshold = low_cov_threshold,
    exceed = FALSE, direction = 1,
    geo = geo_admin2, by.geo = "admin2.name.full", ylim = c(0, 1)
  ) + ggplot2::ggtitle(
    paste0("Cluster-Level Model: Prob. ANC 4+ < ", thr_label)
  ...
  p21 <- surveyPrev::exceedPlot(
    anc_unit_strat_adm1, threshold = low_cov_threshold,
    exceed = FALSE, direction = 1,
    geo = geo_admin1, by.geo = "NAME_2", ylim = c(0, 1)
  ) + ggplot2::ggtitle(
    paste0("Stratified Model: Prob. ANC 4+ < ", thr_label)
  )

  p22 <- surveyPrev::exceedPlot(
    anc_unit_strat_adm2, threshold = low_cov_threshold,
    exceed = FALSE, direction = 1,
    geo = geo_admin2, by.geo = "admin2.name.full", ylim = c(0, 1)
  ) + ggplot2::ggtitle(
    paste0("Stratified Model: Prob. ANC 4+ < ", thr_label)
  ...
  admin1_summary <- data.frame(
    Region          = anc_res_adm1$res.admin1$admin1.name,
    Direct_Est      = round(anc_res_adm1$res.admin1$direct.est, 4),
    Direct_SE       = round(anc_res_adm1$res.admin1$direct.se, 4),
    FH_Est          = round(anc_FH_adm1$res.admin1$mean, 4),
    FH_SE           = round(anc_FH_adm1$res.admin1$sd, 4),
    Unit_Est        = round(anc_unit_adm1$res.admin1$mean, 4),
    Unit_SE         = round(anc_unit_adm1$res.admin1$sd, 4),
    Unit_Strat_Est  = round(subset(anc_unit_strat_adm1$res.admin1, type == "full")$mean, 4),
    Unit_Strat_SE   = round(subset(anc_unit_strat_adm1$res.admin1, type == "full")$sd, 4)
  )
  utils::write.csv(admin1_summary,
                   file = file.path(figs_dir, "admin1_summary.csv"),
                   row.names = FALSE)

  out3 <- anc_unit_adm2$res.admin2[, c("admin2.name.full", "mean", "sd")]
  out4 <- anc_unit_strat_adm2$res.admin2[, c("admin2.name.full", "mean", "sd")]
  ...
  summary_table <- merged_data %>%
    ...
    dplyr::summarize(
      total_women    = dplyr::n(),
      anc4_plus      = sum(value == 1, na.rm = TRUE),
      crude_coverage = round(anc4_plus / total_women, 4),
      .groups        = "drop"
    )
  ...
  invisible(list(
    res_adm0          = anc_res_adm0,
    res_adm1          = anc_res_adm1,
    res_adm2          = anc_res_adm2,
    FH_adm1           = anc_FH_adm1,
    FH_adm2           = anc_FH_adm2,
    unit_adm1         = anc_unit_adm1,
    unit_adm2         = anc_unit_adm2,
    unit_strat_adm1   = anc_unit_strat_adm1,
    unit_strat_adm2   = anc_unit_strat_adm2
  ))
}
