...
This section demonstrates how to generate maps and ridge plots for model comparison. The map examples show posterior means and interval widths, where interval width is defined as the upper interval limit minus the lower interval limit. A CV map is shown once for NMR at admin level 1 as an additional diagnostic.
...
The second set of maps uses `transform = "log1p"` to improve visual contrast when most admin 2 estimates are small and a few areas have much larger values.
